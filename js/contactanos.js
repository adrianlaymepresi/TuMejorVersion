...
function init() {
  ...
  root.renderer.setClearColor('lightgray');
  root.renderer.setPixelRatio(window.devicePixelRatio || 1);
  root.camera.position.set(0, 0, 1300);
...
function createTextAnimation() {
    ...
    // Categorías de tamaños según el ancho de pantalla
    var size;
    if (screenWidth > 1600) {
      size = 14; // Pantallas gigantes (monitores grandes)
    } else if (screenWidth > 1200) {
      size = 12; // Laptops
    } else if (screenWidth > 800) {
      size = 12; // Tablets
    } else if (screenWidth > 500) {
      size = 10; // Celulares comerciales
    } else {
      size = 10; // Celulares mini
    }
  
    var geometry = generateTextGeometry("P R E S I", {
      size: size,
      height: 0,
      font: "droid sans",
      weight: "bold",
